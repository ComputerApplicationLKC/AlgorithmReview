...
// 리뷰레지스터랑 리뷰업데이터에 헤더내용이 들어감 누르면 클릭하면 어떻게 바뀔지 정의한게 밑에 내용.
function HeaderMain() {
        ...
        <nav className="navbar justify-content-center navbar-expand-lg navbar-light bg-white">
          <div className="container-fluid">
            <a className="navbar-brand" href="/">
              <b>AlgorithmReview-LKC</b>
            </a>
            <div className="collapse navbar-collapse" id="navbarSupportedContent">
              <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                {sessionStorage.getItem("access_token") !== null ? (
                  <li className="nav-item">
                    <a className="nav-link" onClick={check}>
                      문제 등록
                    </a>
                  </li>
                ) : null}
                ...
                <a className="nav-link" onClick={() => (window.location.href = "/guest")}>
                  방명록
                </a>
              </ul>
              <form class="d-flex">
                <input
                  ...
                  aria-label="Search"
                />
                <button onClick={onClick} class="btn btn-outline-secondary" type="submit">
                  Search
                </button>
              </form>
            </div>
...
function LoginCheck() {
  ...
  if (sessionStorage.getItem("nickname")) {
    return (
      <Welcome>
        <b>{sessionStorage.getItem("nickname")}</b>님 환영합니다!<Logout onClick={onLogout}>logout</Logout>
      </Welcome>
    );
  }
  return <></>;
